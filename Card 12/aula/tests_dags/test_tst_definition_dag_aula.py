...
def dag(dagbag):
    return dagbag.get_dag('tst_dag') #pega a dag do dagbag usando o id da dag

class TestTstDagDefinition: #classe de teste pra testar a definicao da dag tst_dag
    EXPECTED_NB_TASKS = 6
    EXPECTED_TASKS = ['task_1', 'task_2', 'task_3', 'task_4', 'task_5', 'task_6']
 
    compare = lambda self, x, y: collections.Counter(x) == collections.Counter(y) #ve se as listas tem os mesmo itens

    def test_nb_tasks(self, dag): #funcao de teste q ve o nmr de tasks 
        """
    ...
    def test_contain_tasks(self, dag): #testa se as tasks estao certas
        """
    ...
    @pytest.mark.parametrize("task, expected_upstream, expected_downstream", #roda o teste varias vezes com dados diferentes 
        [
            ("task_1", [], ["task_2"]), 
    ...
    def test_dependencies_of_tasks(self, dag, task, expected_upstream, expected_downstream): #testa as dependencias de cada task
        """
    ...
    def test_start_date_and_catchup(self, dag): #testa start_date e catchup da dag
        """ 
            Verify that the start_date is < current date and catchup = False
        """

    def test_same_start_date_all_tasks(self, dag): #testa se todas as tasks tem a mesma start_date
        """
